...
const distancesFromUser = (user: User, cadets: any) => {
...
const FindandMapCadets = async (req: any, res: any, next:any) => {
    ...
    try {
        ...
        if (cadetsClosest) {
            ...
            await distancesFromUser(user, cadetsClosest)
        }
        ...
        while (!foundEnoughData && nearbyCities.length >= maxSearches) {
            while (index <= maxSearches) {
                ...
                if (cadets) {
                    ...
                    await distancesFromUser(user as User, cadets)
                }
        ...
        // batches to send notifications

        // let batches: CadetInfo[][] = [];

        // let iterator = 0;

        // while (iterator < cadetsList.length) {
        //     let batch:CadetInfo[] = [];
        //     for (let i = 0; i < cadetsList[iterator].length; i++) {
        //         batch = batch.concat(cadetsList[iterator][i].user_id)
        //     }
